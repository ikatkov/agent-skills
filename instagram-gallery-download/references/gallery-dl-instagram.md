# gallery-dl Instagram Notes

## Supported URL shapes

- Post or carousel only: `https://www.instagram.com/p/<shortcode>/`
...
Use the `yt-dlp` skill for reel/reels URLs.

## Useful pass-through flags

Use these after `--` in the helper script.

- `-R 2` retry failed downloads up to 2 times
- `--sleep 1-3` random delay range between requests
- `--range 1-20` limit batch size
- `--filter "extension in ('jpg', 'mp4')"` keep only selected file types

## Authentication guidance

Public post URLs often work without login. Some content may still require cookies.

Browser examples:
- `--cookies-browser chrome`
- `--cookies-browser firefox`
- `--cookies-browser safari`

If browser-cookies access fails, run `gallery-dl --help` to confirm supported cookie import options in the local build.

## Full CLI Options (`gallery-dl --help`)
...
Output Options:
  ...
  -N, --print [EVENT:]FORMAT  Write FORMAT during EVENT (default 'prepare') to standard output instead of downloading files. Can
                              be used multiple times. Examples: 'id' or 'post:{md5[:8]}'
...
Downloader Options:
  ...
  --sleep SECONDS             Number of seconds to wait before each download. This can be either a constant value or a range
                              (e.g. 2.7 or 2.0-3.5)
  --sleep-skip SECONDS        Number of seconds to wait after skipping a file download
...
Selection Options:
  -A, --abort N[:TARGET]      Stop current extractor(s) after N consecutive file downloads were skipped. Specify a TARGET to set
                              how many levels to ascend or to which subcategory to jump to. Examples: '-A 3', '-A 3:2', '-A
                              3:manga'
  -T, --terminate N           Stop current & parent extractors and proceed with the next input URL after N consecutive file
  ...
  --range RANGE               Index range(s) specifying which files to download. These can be either a constant value, range, or
                              slice (e.g. '5', '8-20', or '1:24:3')
  --post-range RANGE          Like '--range', but for posts
  --chapter-range RANGE       Like '--range', but for child extractors handling manga chapters, external URLs, etc.
  --filter EXPR               Python expression controlling which files to download. Files for which the expression evaluates to
  ...
  --chapter-filter EXPR       Like '--filter', but for child extractors handling manga chapters, external URLs, etc.
...
Post-processing Options:
  ...
  --ugoira FMT                Convert Pixiv Ugoira to FMT using FFmpeg. Supported formats are 'webm', 'mp4', 'gif', 'vp8',
  ...
  --exec-after CMD            Execute CMD after all files were downloaded. Example: --exec-after "cd {_directory} && convert *
                              ../doc.pdf"
```
